...
*   **Discovery & Search:** Use `gitlab:search` to find projects, issues, or MRs globally. Use `gitlab:view` to inspect the details of a specific resource before acting on it.
...
- [ ] Step 1: Resource Discovery & Context (using gitlab:* search/view tools)
...
1. If the resource ID is unknown, use `gitlab:search({ "query": "...", "scope": "merge_requests" })`.
2. Use `gitlab:view({ "type": "mr", "id": "<IID>" })` to inspect title, description, and status.
3. Use `gitlab:get_mr_diffs({ "iid": "<IID>" })` to see the actual code changes.
4. Use `gitlab:list_discussions({ "iid": "<IID>", "only_unresolved": true })` to find active threads.
...
*   **Monitor & Wait for CI:**
    Use `gitlab:list_pipelines` followed by `gitlab:wait_for_pipeline` to wait for completion.
