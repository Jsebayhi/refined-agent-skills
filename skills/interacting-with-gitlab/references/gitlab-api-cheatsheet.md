...
1. **`gitlab_get_pull_request_details`**: Fetch MR state and metadata.
2. **`gitlab_add_comment_to_review`**: Add pending feedback (Review Mode).
3. **`gitlab_list_vulnerabilities`**: Audit code for security issues.
...
1. **`gitlab_create_pull_request`**: Open a new Merge Request. Use `fill: true` for automatic metadata.
2. **`gitlab_get_pull_request_details`**: Monitor labels and approval status.
3. **`gitlab_list_workflow_runs`**: Monitor CI execution.
4. **`gitlab_list_vulnerabilities`**: Check for security scan results in CI.
5. **`gitlab_get_job_logs`**: Troubleshoot failed jobs.
...
| :--- | :--- | :--- |
| `400 Bad Request` | Invalid payload or missing fields. | Verify `id`, `path`, and `line` number. |
